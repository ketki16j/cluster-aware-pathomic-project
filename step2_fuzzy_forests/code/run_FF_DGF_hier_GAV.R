...
df   <- read.csv(file.path(WORKDIR, 'data/Renal_Data_hier_GAV.csv'))
...
pathomic_cols <- names(df_t)[grepl('^glomeruli|^arteries|^scl_gloms', names(df_t))]
...
cat('Running FF DGF hier_GAV...\n')
ff  <- wff(X, y, WGCNA_params=wp, screen_params=sp, select_params=lp, final_ntree=1000)
out <- file.path(WORKDIR,'results/fuzzyforest',paste0('FF_DGF_hier_GAV_top100.csv'))
