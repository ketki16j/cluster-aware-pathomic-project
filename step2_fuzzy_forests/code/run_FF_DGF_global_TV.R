...
df   <- read.csv(file.path(WORKDIR, 'data/Renal_Data_cluster_averaged_TV.csv'))
...
pathomic_cols <- names(df_t)[grepl('^T_|^V_', names(df_t))]
...
cat('Running FF DGF global_TV...\n')
ff  <- wff(X, y, WGCNA_params=wp, screen_params=sp, select_params=lp, final_ntree=1000)
out <- file.path(WORKDIR,'results/fuzzyforest',paste0('FF_DGF_global_TV_top100.csv'))
