...
df   <- read.csv(file.path(WORKDIR, 'data/Renal_Data_cluster_averaged_GTAV.csv'))
full <- read.csv(file.path(WORKDIR, 'data/Renal_Data.csv'))
has_outcome <- !is.na(full$Delayed_Graft_Function)
...
y <- as.numeric(full_t$Delayed_Graft_Function)
...
pathomic_cols <- names(df_t)[grepl('^G_|^T_|^A_|^V_', names(df_t))]
...
cat('Running FF DGF global_GTAV...\n')
ff  <- wff(X, y, WGCNA_params=wp, screen_params=sp, select_params=lp, final_ntree=1000)
out <- file.path(WORKDIR,'results/fuzzyforest',paste0('FF_DGF_global_GTAV_top100.csv'))
